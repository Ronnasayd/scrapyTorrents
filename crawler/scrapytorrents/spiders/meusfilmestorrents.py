...
class MeusfilmestorrentsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        next_page = response.css(
            '#content > div > div > a.nextpostslink::attr(href)').get()
        yield response.follow(next_page, callback=self.parse)

    def getData(self, response):
        info = response.css(
            'div.entry-content.cf').get()
        infoText = BeautifulSoup(info).get_text()
        infoDictionay = handleParserInfo(self, infoText)
        torrent = ScrapytorrentsItem(**infoDictionay)

        torrent['image_url'] = response.css(
            'div.entry-content.cf img::attr(src)').get(default="N/F")
        torrent['trailer'] = response.css(
            'iframe::attr(src)').get(default="N/A")
        torrent['original_link'] = response.url

        magnet_links = response.css(
            'div.entry-content.cf a[href^="magnet"]::attr(href)').getall()
